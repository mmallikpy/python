...
def arpToSwitchPortDescription(ip):
    ...
    for x in swli:
        ...
        for inter in accessPorts:
            output = ssh.send_command(f"show running-config interface {inter}", use_textfsm=True)
        ...
        for line in x:
            ...
            for interf in finalAccessPorts:
                macinter = ssh.send_command(f"show mac address-table interface {interf}", use_textfsm=True)
